...
/* Builder Constructor */
template <typename T>
Vertices2<T>::Vertices2(std::vector<Vector2<T>> _vertices) {
    vertices = {};
    for (int i = 0; i < _vertices.size(); i++) { vertices.push_back(std::make_shared<Vector2<T>>(_vertices[i])); }
}
template Vertices2<int>::Vertices2(std::vector<Vector2<int>> vertices);
template Vertices2<float>::Vertices2(std::vector<Vector2<float>> vertices);
template Vertices2<double>::Vertices2(std::vector<Vector2<double>> vertices);

/* Builder Constructor */
template <typename T>
Vertices2<T>::Vertices2(std::vector<std::shared_ptr<Vector2<T>>> _vertices) {
    vertices = _vertices;
}
template Vertices2<int>::Vertices2(std::vector<std::shared_ptr<Vector2<int>>> vertices);
template Vertices2<float>::Vertices2(std::vector<std::shared_ptr<Vector2<float>>> vertices);
template Vertices2<double>::Vertices2(std::vector<std::shared_ptr<Vector2<double>>> vertices);
...
template <typename T>
std::vector<std::pair<std::shared_ptr<Vector2<T>>, std::shared_ptr<Vector2<T>>>> Vertices2<T>::pairs() const {
    std::vector<std::pair<std::shared_ptr<Vector2<T>>, std::shared_ptr<Vector2<T>>>> pairs = {};
    int vertices_size = vertices.size();
    for (int i = 0; i < vertices_size; i++) {
        std::shared_ptr<Vector2<T>> pA = vertices[i];
        std::shared_ptr<Vector2<T>> pB = vertices[com::modulo(i + 1, vertices_size)];
        pairs.push_back({pA, pB});
    }
    return pairs;
}
template std::vector<std::pair<std::shared_ptr<Vector2<int>>, std::shared_ptr<Vector2<int>>>> Vertices2<int>::pairs() const;
template std::vector<std::pair<std::shared_ptr<Vector2<float>>, std::shared_ptr<Vector2<float>>>> Vertices2<float>::pairs() const;
template std::vector<std::pair<std::shared_ptr<Vector2<double>>, std::shared_ptr<Vector2<double>>>> Vertices2<double>::pairs() const;

/* Check if the Polygon is Self Intersecting */
template <typename T>
bool Vertices2<T>::intersect() const {
    std::vector<std::pair<std::shared_ptr<Vector2<T>>, std::shared_ptr<Vector2<T>>>> sides = pairs();
    ...
    for (int i = 0; i < sides.size() - 2; i++) {
        for (int j = i + 2; j < sides.size() - (i == 0); j++) {
            std::pair<std::shared_ptr<Vector2<T>>, std::shared_ptr<Vector2<T>>> sideA = sides[i];
            std::pair<std::shared_ptr<Vector2<T>>, std::shared_ptr<Vector2<T>>> sideB = sides[j];
...
bool Vertices2<T>::convex() const {
    ...
    for (int i = 0; i < sides.size() - 2; i++) {
        for (int j = i + 2; j < sides.size() - (i == 0); j++) {
            std::pair<std::shared_ptr<Vector2<T>>, std::shared_ptr<Vector2<T>>> sideA = sides[i];
            std::pair<std::shared_ptr<Vector2<T>>, std::shared_ptr<Vector2<T>>> sideB = sides[j];
            if (Vector2<T>::segments_intersect(*sideA.first, *sideA.second, *sideB.first, *sideB.second)) { return false; }
        }
    }
    ...
    for (int i = 1; i < sides.size(); i++) {
        ...
        bool corientation = Vector2<T>::cross(last, curr) > T(0);
...
template <typename T>
std::vector<T> Vertices2<T>::sizes() const {
    std::vector<T> sizes = {};
    int vertices_size = vertices.size();
    for (int i = 0; i < vertices_size; i++) {
        Vector2<T> pA = *vertices[i];
...
template std::vector<int> Vertices2<int>::sizes() const;
template std::vector<float> Vertices2<float>::sizes() const;
template std::vector<double> Vertices2<double>::sizes() const;

/* Return the bounds of the polygon */
template <typename T>
Bounds<T> Vertices2<T>::limits() const {
    const auto min_max_x = std::minmax_element(vertices.begin(), vertices.end(), [](const std::shared_ptr<Vector2<T>>& lhs, const std::shared_ptr<Vector2<T>>& rhs) { return (*lhs).x < (*rhs).x; });
    const auto min_max_y = std::minmax_element(vertices.begin(), vertices.end(), [](const std::shared_ptr<Vector2<T>>& lhs, const std::shared_ptr<Vector2<T>>& rhs) { return (*lhs).y < (*rhs).y; });

    std::shared_ptr<Vector2<T>> min_x_ptr = *min_max_x.first;
    std::shared_ptr<Vector2<T>> max_x_ptr = *min_max_x.second;
    std::shared_ptr<Vector2<T>> min_y_ptr = *min_max_y.first;
    std::shared_ptr<Vector2<T>> max_y_ptr = *min_max_y.second;
...
void Vertices2<T>::rotate(const T& rotation, const Vector2<T>& centroid) {
    std::vector<std::shared_ptr<Vector2<T>>> temp = {};
...
/* Ear Clipping */
template <typename T>
std::vector<Vertices2<T>> Vertices2<T>::triangulate() const {
    std::vector<Vertices2<T>> triangles = {};
    ...
    while (cvertices.vertices.size() >= 3) {
        ...
        for (int i = 0; i < psize; i++) {
            std::shared_ptr<Vector2<T>> pA = cvertices.vertices[i];
            std::shared_ptr<Vector2<T>> pB = cvertices.vertices[com::modulo(i + 1, psize)];
            std::shared_ptr<Vector2<T>> pC = cvertices.vertices[com::modulo(i + 2, psize)];
...
template std::vector<Vertices2<int>> Vertices2<int>::triangulate() const;
template std::vector<Vertices2<float>> Vertices2<float>::triangulate() const;
template std::vector<Vertices2<double>> Vertices2<double>::triangulate() const;
...
bool Vertices2<T>::point_in_shape(const Vertices2<T>& vertices, const Vector2<T>& point) {
    ...
    const auto max_x =
        std::max_element(vertices.vertices.begin(), vertices.vertices.end(), [](const std::shared_ptr<Vector2<T>>& lhs, const std::shared_ptr<Vector2<T>>& rhs) { return (*lhs).x < (*rhs).x; });
...
template <typename T>
std::pair<std::shared_ptr<Vector2<T>>, std::shared_ptr<Vector2<T>>> Vertices2<T>::closest_edge(const Vertices2<T>& vertices, const Vector2<T>& point) {
    std::vector<std::pair<std::shared_ptr<Vector2<T>>, std::shared_ptr<Vector2<T>>>> pairs = vertices.pairs();
    const auto closest =
        std::min_element(pairs.begin(),
                         pairs.end(),
                         [point](const std::pair<std::shared_ptr<Vector2<T>>, std::shared_ptr<Vector2<T>>>& lhs, const std::pair<std::shared_ptr<Vector2<T>>, std::shared_ptr<Vector2<T>>>& rhs) {
    ...
    std::pair<std::shared_ptr<Vector2<T>>, std::shared_ptr<Vector2<T>>> closest_side = (*closest);
    return closest_side;
}
template std::pair<std::shared_ptr<Vector2<int>>, std::shared_ptr<Vector2<int>>> Vertices2<int>::closest_edge(const Vertices2<int>& vertices, const Vector2<int>& point);
template std::pair<std::shared_ptr<Vector2<float>>, std::shared_ptr<Vector2<float>>> Vertices2<float>::closest_edge(const Vertices2<float>& vertices, const Vector2<float>& point);
template std::pair<std::shared_ptr<Vector2<double>>, std::shared_ptr<Vector2<double>>> Vertices2<double>::closest_edge(const Vertices2<double>& vertices, const Vector2<double>& point);
