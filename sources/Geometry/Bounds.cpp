...
/* Builder Constructor */
template <typename T>
Bounds<T>::Bounds(Vector2<T> _p1, Vector2<T> _p2) {
    p1 = _p1;
    p2 = _p2;
}
template Bounds<int>::Bounds(Vector2<int> p1, Vector2<int> p2);
template Bounds<float>::Bounds(Vector2<float> p1, Vector2<float> p2);
template Bounds<double>::Bounds(Vector2<double> p1, Vector2<double> p2);

/* Builder Constructor */
template <typename T>
Bounds<T>::Bounds(T x1, T y1, T x2, T y2) {
    p1 = Vector2<T>(x1, y1);
    p2 = Vector2<T>(x2, y2);
}
template Bounds<int>::Bounds(int x1, int y1, int x2, int y2);
template Bounds<float>::Bounds(float x1, float y1, float x2, float y2);
template Bounds<double>::Bounds(double x1, double y1, double x2, double y2);
...
Bounds<T> Bounds<T>::operator+(const Bounds<T>& other) const {
    return Bounds<T>(p1.x + other.p1.x, p1.y + other.p1.y, p2.x + other.p2.x, p2.y + other.p2.y);
}
...
Bounds<T> Bounds<T>::operator*(const T& other) const {
    return Bounds<T>(p1.x * other, p1.y * other, p2.x * other, p2.y * other);
}
...
bool Bounds<T>::operator==(const Bounds<T>& other) const {
    if (p1.x != other.p1.x) { return false; }
    if (p1.y != other.p1.y) { return false; }
    if (p2.x != other.p2.x) { return false; }
    if (p2.y != other.p2.y) { return false; }
    return true;
}
...
Vector2<T> Bounds<T>::size() const {
    return Vector2<T>(p2.x - p1.x, p2.y - p1.y);
}
...
Bounds<T> Bounds<T>::reorder() const {
    com::vec<Vector2<T>> points = {Vector2<T>(p1.x, p1.y), Vector2<T>(p2.x, p2.y)};
...
Bounds<T> Bounds<T>::clone() const {
    return Bounds<T>(p1.x, p1.y, p2.x, p2.y);
}
...
bool Bounds<T>::point_in_bounds(const Vector2<T>& v1, const Bounds<T>& b2) {
    if (v1.x < b2.p1.x) { return false; }
    if (v1.x > b2.p2.x) { return false; }
    if (v1.y < b2.p1.y) { return false; }
    if (v1.y > b2.p2.y) { return false; }
    return true;
}
...
bool Bounds<T>::circle_in_bounds(const T& r, const Vector2<T>& v1, const Bounds<T>& b2) {
    if (v1.x - r < b2.p1.x) { return false; }
    if (v1.x + r > b2.p2.x) { return false; }
    if (v1.y - r < b2.p1.y) { return false; }
    if (v1.x + r > b2.p2.y) { return false; }
    return true;
}
...
bool Bounds<T>::segment_intersect_bounds(const Vector2<T>& v1, const Vector2<T>& v2, const Bounds<T>& b2) {
    if (Vector2<T>::segments_intersect(v1, v2, Vector2<T>(b2.p1.x, b2.p1.y), Vector2<T>(b2.p2.x, b2.p1.y))) { return true; }
    if (Vector2<T>::segments_intersect(v1, v2, Vector2<T>(b2.p2.x, b2.p1.y), Vector2<T>(b2.p2.x, b2.p2.y))) { return true; }
    if (Vector2<T>::segments_intersect(v1, v2, Vector2<T>(b2.p2.x, b2.p2.y), Vector2<T>(b2.p1.x, b2.p2.y))) { return true; }
    if (Vector2<T>::segments_intersect(v1, v2, Vector2<T>(b2.p1.x, b2.p2.y), Vector2<T>(b2.p1.x, b2.p1.y))) { return true; }
...
bool Bounds<T>::bounds_in_bounds(const Bounds<T>& b1, const Bounds<T>& b2) {
    if (Bounds<T>::point_out_bounds(Vector2<T>(b1.p1.x, b1.p1.y), b2)) { return false; }
    if (Bounds<T>::point_out_bounds(Vector2<T>(b1.p1.x, b1.p2.y), b2)) { return false; }
    if (Bounds<T>::point_out_bounds(Vector2<T>(b1.p2.x, b1.p1.y), b2)) { return false; }
    if (Bounds<T>::point_out_bounds(Vector2<T>(b1.p2.x, b1.p2.y), b2)) { return false; }
    return true;
}
...
bool Bounds<T>::bounds_intersect_bounds(const Bounds<T>& b1, const Bounds<T>& b2) {
    if (Bounds<T>::segment_intersect_bounds(Vector2<T>(b1.p1.x, b1.p1.y), Vector2<T>(b1.p2.x, b1.p1.y), b2)) { return true; }
    if (Bounds<T>::segment_intersect_bounds(Vector2<T>(b1.p2.x, b1.p1.y), Vector2<T>(b1.p2.x, b1.p2.y), b2)) { return true; }
    if (Bounds<T>::segment_intersect_bounds(Vector2<T>(b1.p2.x, b1.p2.y), Vector2<T>(b1.p1.x, b1.p2.y), b2)) { return true; }
    if (Bounds<T>::segment_intersect_bounds(Vector2<T>(b1.p1.x, b1.p2.y), Vector2<T>(b1.p1.x, b1.p1.y), b2)) { return true; }
